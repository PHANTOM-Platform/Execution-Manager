...
# 3. ##################  DELETE DATABASE  ###################################
	echo -e "\n${LIGHT_BLUE}"; 
	echo "curl -s -XGET http://${server}:${appmanager_port}/drop_db";
	read -p $'Do you wish to \033[1;37mDELETE\033[1;34m the Repository \033[1;37mDB\033[1;34m? (y/n)' confirm; echo -ne "${NO_COLOUR}";
	if [[ ! ${confirm} = "" ]]; then
		if [ ${confirm} == 'y' ] || [ ${confirm} == 'Y' ];then
			HTTP_STATUS=$(curl  --silent --output /dev/null --write-out "%{http_code}" -XGET http://${server}:${appmanager_port}/drop_db);
			if [[ ${HTTP_STATUS} != "200" ]]; then
...
# 4. ##################  CREATE A NEW DATABASE  ###################################
	echo -e "\n${LIGHT_BLUE}"; 
	echo "curl -s -XGET http://${server}:${appmanager_port}/new_db";
	read -p $'Press [Enter] key to \033[1;37mCREATE\033[1;34m a new empty Repository \033[1;37mDB\033[1;34m'; echo -ne "${NO_COLOUR}"; 
	HTTP_STATUS=$(curl  --silent --output /dev/null --write-out "%{http_code}" -XGET http://${server}:${appmanager_port}/new_db);
	if [[ ${HTTP_STATUS} != "200" ]]; then
		echo "> Error, can not create DBs. ";
		curl  --silent -XGET http://${server}:${appmanager_port}/new_db;
	else
